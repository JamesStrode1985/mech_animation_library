...
ctx=bpy.app.driver_namespace['HC animation build context'];dc=bpy.app.driver_namespace['HC dance context']
...
try:
    ctx['reset']();rig.animation_data.action=bpy.data.actions['HC ANIM | 46 Dance Party']
...
report={'clip':'46 Dance Party','frames_checked':193,'root_error':root,'minimum_body_surface_z':minimum,'maximum_planted_foot_slide':slide,'maximum_joint_attachment_error':attachment,'loop_seam':seam,'intersections':hits,'scope':'All frames: stationary root, conservative body floor clearance, planted-foot movement, leg joint attachment, loop endpoints, selected leg armor pairs and moving pod armor against the hull. Not an exhaustive all-object collision check.'}
(Path(PROJECT_ROOT)/'data/verification/dance_party.json').write_text(json.dumps(report,indent=2)+'\n')
