"""Render requested death preview frames through Blender MCP; restore scene settings.
Supply PROJECT_ROOT, DEATH_LABELS and optionally DEATH_FRAMES.
"""
...
out=Path(PROJECT_ROOT)/'build/death_previews';out.mkdir(parents=True,exist_ok=True)
...
try:
    ...
    for c in dc['config']:
        if c['label'] not in DEATH_LABELS:continue
        ...
        for f in [1,c['begin']+1,(c['begin']+c['impact'])//2+1,c['impact']+1,c['duration']+1]:
            studio.frame_set(f)
        ...
        frames=globals().get('DEATH_FRAMES') or list(range(1,c['duration']+2,2))
        for f in frames:
