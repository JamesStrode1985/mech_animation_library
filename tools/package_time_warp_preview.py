"""Package Time Warp for the offline gallery. Requires Pillow."""
import json
...
def main():
    clip=json.loads((ROOT/'build/time_warp_action.json').read_text())
    report=json.loads((ROOT/'data/verification/time_warp.json').read_text())
    ...
    frames=[]
    for f in range(1,265,2):
        with Image.open(ROOT/'build/time_warp_previews/47_time_warp'/f'frame_{f:03d}.png') as image:
            im=image.convert('RGB');im.thumbnail((540,480),Image.Resampling.LANCZOS);frames.append(im.copy())
    atlas=Image.new('RGB',(900,160))
    for j,k in enumerate([12,26,49,74,118]):
    ...
    data['time_warp_verification']=report
    ...
    print('Packaged 47 Time Warp: 132 preview frames, 11 seconds; approximate soundtrack tempo.')
