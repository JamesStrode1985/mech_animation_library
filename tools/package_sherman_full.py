...
def main():
    ...
    selected=set(map(int,sys.argv[1:]));existing={c['slug']:c for c in manifest['clips']}
    clips=manifest['clips'][:3];reports={}
    for clip,source in zip(clips,original['clips'][:3]):clip['hellcat_source_action']=source['action']
    fields=['label','source_label','action','slug','duration_frames','fps','loop','category','gallery_group','markers','revision','preview_frame_step','controller_preview_travel','hellcat_source_action','nominal_speed_units_per_s','terrain_mode','terrain_period','timing_note','controller_distance_units','controller_direction','controller_travel_keys','controller_interpolation']
    for spec in specs:
        ...
        reports[slug]=check
        if selected and spec['number'] not in selected:
            clips.append(existing[slug]);continue
        step=spec['preview_frame_step'];preview_fps=24/step
        frame_numbers=list(range(1,spec['duration_frames']+1,step))
        ...
        durations=[round((i+1)*100/preview_fps)*10-round(i*100/preview_fps)*10 for i in range(len(frames))]
    ...
    result=dict(mech_id='sherman-walker',revision=3,gallery_groups=original['gallery_groups'],clips=clips,verification_report='data/verification/sherman_full_library.json')
