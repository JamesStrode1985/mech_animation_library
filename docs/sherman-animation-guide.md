...
Clips 01–03 have 24 fps GIF previews. Clips 04–47 sample the 24 fps Actions at 12 fps,
...
4. `blender_render_sherman_full.py` accepts `RENDER_NUMBER` and bounded
   `RENDER_FRAMES`; render every other frame and the final key for one-shots.
   Temporary PNGs go under ignored `build/sherman_full/`.
5. Run `python tools/package_sherman_full.py` with Pillow. It validates reports
